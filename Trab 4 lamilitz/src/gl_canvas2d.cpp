...
void CV::point(Vector2 p, float size)
{
   glPointSize(size);
   glBegin(GL_POINTS);
      glVertex2d(p.x, p.y);
...
void CV::line( float x1, float y1, float x2, float y2, float thickness )
{
...
void CV::line( Vector2 p1, Vector2 p2, float thickness )
{
   glLineWidth(thickness);
   glBegin(GL_LINES);
...
void CV::circle( float x, float y, float radius, int div, float thickness )
{
   float ang = 0, x1, y1;
   float inc = PI_2/div;
   glLineWidth(thickness);
...
void CV::circle( Vector2 v, float radius, int div, float thickness )
{
   float ang = 0, x1, y1;
   float inc = PI_2/div;
   glLineWidth(thickness);
